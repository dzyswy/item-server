...
async_client::async_client(std::string ip, int port, std::string request)
	: socket_(io_context_)
{
	request_s_ = request;
...
void async_client::do_connect(ip::tcp::endpoint &ep)
{
...
void async_client::do_write_request()
{
...
void async_client::do_read_status_line()
{
...
void async_client::do_read_headers()
{
	asio::async_read_until(socket_, response_, "\r\n\r\n",
        [this] (asio::error_code ec, std::size_t n) 
		{
...
int async_client::get_result(std::string &result, int timeout)
{
	std::unique_lock<std::mutex> lock(mux_);
